...
# другой тип опасных космических обломков. Необходимо, чтобы какое-либо качество отличало эту новую
# разновидность от астероидов: например, для разрушения обломков такого типа может понадобиться два
# ракетных залпа вместо одного.

# Description:
# В игру добавляется новый тип астероида, игра в начале каждого уровня определяет рандомно какой астероид будет создан
# Новый астероид уничтожается после двух выстрелов
# Смог добиться минимального обрамления рамки
# Ввел в игру металический астероид. Теперь надо придумать логику попадания. Металический астероид переживает 2 попадания

# Pseudocode:
# Расширяем класс Астероид, добавляя в него новую картинку и новую логику:
# Счетчик попаданий, если счетчик равен 2, тогда астероид умирает
# Новая логика скорости, она будет всегда одинаковой, равна 3
# Добавить в класс астероидов счетчик прочности, если он равен 0, астероид взрывается
...
class Asteroid(Wrapper): # Класс астероид наследуется от класса Wrapper который позволяет огибать экран
    ...
    images = {SMALL: games.load_image('asteroid_small.bmp'),
             ...
             METALL: games.load_image('metal_asteroid.png')}
    ...
    def __init__(self, game, x, y, size, durability): # game - Это экз Game, x и y - это местоположение, size - число от 1 до 3, это размер астероида
        ...
        self.game = game
        self.durability = durability # Прочность создаваемого астероида


    def die(self): # В случае применения к экземпляру класса Asteroid:
        """Destroy asteroid and replace some asteroid on 2 less"""
        if self.size != Asteroid.SMALL and self.size != Asteroid.METALL: # Проверка на размер, если размер не равен 1
...
class Game():
    ...
    def advance(self): # Продвижение на новый уровень
        ...
        for i in range(self.level): # В зависимости от уровня
            ...
            if rand_numb == 1: # Определяет какой астероид появится, который делится или металический
